...
export default function OrderConfirmation({ orderId }: Props) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16 text-center">
      ...
      <p className="text-xs text-gray-400 mb-4 font-mono break-all">#{orderId}</p>
      {emailSent && <p className="text-xs text-green-600 mb-2">Te enviamos un correo de confirmación.</p>}
      {emailError && <p className="text-xs text-gray-400 mb-2">No pudimos enviar el correo de confirmación.</p>}
      {whatsappSent && <p className="text-xs text-green-600 mb-8">Te enviamos un resumen por WhatsApp.</p>}
      <Link href="/" className="inline-block bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold text-sm">
        Volver al inicio
      </Link>
    </div>
  )
