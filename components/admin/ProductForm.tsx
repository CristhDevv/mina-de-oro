...
import { 
  ...
  Heading3, Bold, ImageIcon, Eye, Settings, Layout
} from 'lucide-react'
...
export default function ProductForm({ product, categories, onCancel, onSaved }: Props) {
  ...
  const [landingConfig, setLandingConfig] = useState<LandingConfig>(
    (product?.landing_config as LandingConfig) ?? {}
  )
    ...
    <div className="w-full min-h-screen bg-[#F8F9FD]">
      <div className="w-full max-w-3xl mx-auto flex flex-col">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Section: Landing Config */}
          <section className="bg-white p-8 border-b border-gray-100 space-y-6">
            <SectionHeader
              icon={Layout}
              title="Landing Page — Colores y Secciones"
              description="Personaliza la página de producto. Los cambios se reflejan en tiempo real. Los productos sin configuración usan los valores por defecto."
            />
            <LandingConfigEditor
              value={landingConfig}
